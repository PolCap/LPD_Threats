...
# Figure S2: Habitat breadth among groups ######################################
...
# Figure S4: Human density vs latitude #########################################

(figureS4 <- lpi %>% 
   distinct(ID, .keep_all=T) %>% 
   ggplot(aes(abs(Latitude), log(pop_dens+1))) +
   geom_point(alpha=0.2)+
   stat_smooth(method="lm")+
   labs(x="Absolute latitude", y="Log(Human density)"))

ggsave(filename = "Fig. S4.pdf", figureS4,
       path = ResultPath,
       height = 6, width = 6)


# Table S3: General models #####################################################

tableS3 <- rbind(describe_posterior(sys_1),
...
# Table S4: Specific body mass models ##########################################

tableS4 <- rbind(describe_posterior(bm_amp_fre), 
...
# Table S5: Specific latitude models ##########################################

tableS5 <- rbind(describe_posterior(lat_amp_fre), 
